...
import { disposeIsland, makeIsland, world } from './island-scene';

type Runtime = { scene: THREE.Scene; camera: THREE.OrthographicCamera; controls: OrbitControls; render: () => void };
export default function IslandCanvas({ board, preview = false, children, fallback }: { board: GameBoard; preview?: boolean; children?: React.ReactNode; fallback?: React.ReactNode }) {
  ...
  useEffect(()=>{
    ...
    scene.add(new THREE.HemisphereLight('#fff8ed','#315568',1.7));
    const sun=new THREE.DirectionalLight('#fff0cf',3);sun.position.set(-5,12,6);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);
    ...
    const fill=new THREE.DirectionalLight('#b9dbff',.8);fill.position.set(6,5,-6);scene.add(fill);
    ...
    runtime.current={scene,camera,controls,render};
    controls.addEventListener('change',render);
    const observer=new ResizeObserver(resize);observer.observe(element);resize();
    let frame=0;let stopped=false;const animate=()=>{if(stopped)return;controls.update();frame=requestAnimationFrame(animate);};animate();
    const lost=(e:Event)=>{e.preventDefault();setFailed(true);};renderer.domElement.addEventListener('webglcontextlost',lost);
    return()=>{stopped=true;cancelAnimationFrame(frame);observer.disconnect();controls.dispose();renderer.domElement.removeEventListener('webglcontextlost',lost);renderer.dispose();renderer.domElement.remove();runtime.current=null;};
  },[]);
  useEffect(()=>{
    const rt=runtime.current;if(!rt)return;const island=makeIsland(board,preview);rt.scene.add(island);rt.render();
    return()=>{rt.scene.remove(island);disposeIsland(island);};
  },[board,preview]);
  useEffect(()=>{runtime.current?.render();},[children]);
